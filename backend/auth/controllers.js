...
module.exports.login = async (req, res) => {
    ...
    try {
        ...
        var queriedUser = await User.findOne({ username })
        ...
        if (queriedUser.checkPassword(password) == false) {
...
// Discord stuff
module.exports.discordAuthCallback = async (req, res) => {
    if (!req.query.code) {
    ...
    try {
        ...
        res.status(200).send({ message: 'Created account with Discord' })
    } catch (error) {
